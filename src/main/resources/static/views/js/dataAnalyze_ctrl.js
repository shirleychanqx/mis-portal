...
angular.module('misApp').controller('dataAnalyzeAddController', function($scope,$http,$routeParams,$location) {
	...
	var baseUrl = "http://" + $location.host() + ":" + $location.port() + "/admins/dataAnalyze/" + id;
	if(typeof(id) != "undefined") {
		$scope.id = id;
		$http.get(baseUrl)
		.success(function (response) {
							$scope.id = response.dataAnalyze.id;
							$scope.dataAnalyzeCatId = response.dataAnalyze.dataAnalyzeCatId;
	...
	$scope.dataAnalyze_save = function(){
		...
		if (typeof(id) == "undefined") { 
			var data = {};
							data.id = $scope.id;
							data.dataAnalyzeCatId = $scope.dataAnalyzeCatId;
